...
name=$(curl -s -X POST http://127.0.0.1:6333/collections/freedom_episodic/snapshots | jq -r .result.name)
curl -s http://127.0.0.1:6333/collections/freedom_episodic/snapshots/$name -o offsite-staging/qdrant_$ts.snapshot

# sanity: file non vuoti
test -s offsite-staging/pg_$ts.sql.gz
test -s offsite-staging/qdrant_$ts.snapshot
